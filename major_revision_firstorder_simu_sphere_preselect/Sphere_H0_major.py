...
#     pval2 = np.zeros(M)
    ...
    #bp = gs.eye(d)
    bp0 = np.array([0, 0, 0, 0, 0, 0, 1])
    data = generate_ar_ts(base=bp0 ,n_samples=nsamples,sigma=1,rho=0.05,tau=tau)
   ...
   # windows =np.linspace(0.02, 0.05, num=20) * nsample= 400
    ...
    mean = np.array([0, 0, 0, 1/2, 1/2, 1/2, 1/2]) # empirical frechet mean
...
for i in range(M):
    pval1[i]= res[i][0]
    pval2[i]= res[i][1]
np.save('pval_debias' +'_H0'+'_T_'+str(s1)+'.npy', pval1)
np.save('pval_bias' + '_H0'+'_T_'+str(s1)+'.npy', pval2)
end1=time.time()
print(end1-start1)
